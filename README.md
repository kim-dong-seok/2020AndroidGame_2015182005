...
|6/10(수)~|스테이지 구현|에너미 웨이브, 스테이지 이동 구현|0% (계획 변경 에너미 웨이브 삭제(계속적으로 보충 80%)|
|6/12(금)~|ui, 상호작용 오브젝트 구현|ui 리소스 수집,구현 및 캐릭터와 연동<br>상호작용 오브젝트 클래스 구현|50% (계속적으로 보충 100%)|
|6/13(토)~|주간 점검|미흡한 부분 보충|ui구현 추가 진행 및 리소스 수집,제작|
|6/15(월)~|추가구현 및 디버그|진행 상황에 따라 추가|ui구현 추가 진행 및 리소스 수집,제작|
|6/17(수)~|추가구현 및 디버그|진행 상황에 따라 추가|사운드 추가 및 리소스 수집,제작|
|6/19(금)~|추가구현 및 디버그|진행 상황에 따라 추가|스토리, 스테이지 추가, 포션 추가 등 전반적 진행|
