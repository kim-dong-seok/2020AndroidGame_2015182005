package kr.ac.kpu.game.tiledsample;
...
import kr.ac.kpu.game.tiledsample.tile.TiledMap;
...
//        Point screenSize = new Point();
        ...
        int tileSize = screenSize.x / 10;
        Log.d(TAG, "dstTileWith=" + tileSize);
        if (screenSize.x % 10 > 0) {
            tileSize++;
            Log.d(TAG, "dstTileWith=" + tileSize);
        }
        map = TiledMap.load(getResources(), R.raw.ex_map, true);
        map.loadImage(getResources(), new int[] { R.mipmap.dragon_road_tiles }, tileSize, tileSize);
        Log.d(TAG, "Done?");
